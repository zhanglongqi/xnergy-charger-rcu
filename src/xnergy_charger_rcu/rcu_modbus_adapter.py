...
class RCUModbusAdapter:
	...
	def connect(self):
		try:
			...
			firmware_version_number_reg = self._instrument.read_registers(registeraddress=_MODBUS_ADDRESS_FIRMWARE_VERSION_NUMBER,
	...
	def get_rcu_status(self):
		...
		try:
			data = self._instrument.read_registers(registeraddress=0, number_of_registers=_MODBUS_NUM_REGISTERS, functioncode=_MODBUS_READ_INPUT_REGISTERS)
		except serial.serialutil.SerialException as e:
