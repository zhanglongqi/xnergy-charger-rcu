...
class RCUCANbusAdapter:
	...
	def connect(self):
		try:
			self.bus = can.Bus(interface='socketcan', channel=self._port, receive_own_message=True)
	...
	def get_rcu_status(self):
		...
		try:
			...
			charge_current_msg = charge_current_msg[10:12] + charge_current_msg[8:10]
			self.output_current = int(charge_current_msg[:4], 16)
